...
def wl_jk_read(dataset='',rand_dataset=None,rand_home=None,auto_cross='_wl',bins='bins',home='',calib_bias=0,
               rand_ratio=0,njk=100,**kwargs):
    if rand_dataset is None:
        rand_dataset=dataset+'_rand'
    if rand_home is None:
        rand_home=home
    data_jk=jk_read(dataset=dataset,home=home,auto_cross=auto_cross,corr='',njk=njk,**kwargs)
    rand_jk=jk_read(dataset=rand_dataset,home=rand_home,auto_cross=auto_cross,corr='',
                    njk=njk,**kwargs)
...
def rotate_mask(mask=[],nside=512,frame_in='galactic',frame_out='',nest=False,nside_smooth_fact=4):
    ...
    if frame_out=='icrs':
        ...
        lm3[gpix]=lm2
        lm4=hp.pixelfunc.ud_grade(np.float64(lm3),nside_out=np.int(nside/nside_smooth_fact),pess=False,order_in='Ring',                                                                                        
                              order_out=order_out)
        x=lm4>0
        lm4=x
...
def make_mask(data=[],nside=512,nest=False):
    c=SkyCoord(ra=data['RA']*units.degree, dec=data['DEC']*units.degree,frame='icrs')
    dat_galactic=c.galactic
    gpix=hp.pixelfunc.ang2pix(nside=nside,nest=nest,theta=dat_galactic.b.radian*-1+np.pi/2.,
                              phi=dat_galactic.l.radian)
